...
export function AdminTeamPage({ navigate }: AdminTeamPageProps) {
  ...
  const handlePromote = async () => {
    ...
    const { error } = await supabase
      .from('profiles')
      .update({ role: 'admin', is_super_admin: false })
      .eq('id', searchResult.id);
    if (error) {
      toast.error('Failed to add assistant admin');
      return;
    }
    toast.success(`${searchResult.full_name || searchResult.email} is now an assistant admin`);
    setSearchResult(null);
    setSearchEmail('');
    loadAdmins();
  };
  ...
  const handleRemove = async (admin: AdminProfile) => {
    if (!confirm(`Remove admin access for ${admin.full_name || admin.email}?`)) return;
    const { error } = await supabase
      .from('profiles')
    ...
    <div className="min-h-screen bg-muted/20">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 rounded-xl border border-border/60 bg-card p-5">
          <h2 className="mb-3 text-lg font-bold">Add Assistant Admin</h2>
          ...
          {searchResult && (
            <div className="mt-4 flex items-center justify-between rounded-lg border border-border/60 bg-muted/30 p-4">
              ...
              <Button onClick={handlePromote} disabled={searchResult.role === 'admin'}>
                <UserPlus className="mr-1.5 h-4 w-4" /> Add as Assistant Admin
              </Button>
            </div>
          )}
        </div>
